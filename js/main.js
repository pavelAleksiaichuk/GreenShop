...
window.onload = function () {
	...
	function documentActions(e) {
		...
		} else if(!targetElement.closest('.header__search-form') && !targetElement.closest('.header__search') && document.querySelector('.header__search-form._active')) {
			headerForm.classList.remove('_active');
		}
		// Show/Hide Login
		const loginBox = document.querySelector('.header__login-forms');
		if(targetElement.closest('.header__login')) {
			e.preventDefault();
			loginBox.classList.add('_show');
		} else if(targetElement.closest('.login-box__close')) {
			loginBox.classList.remove('_show');
		} else if((!targetElement.closest('.login-box__inner') && !targetElement.closest('.header__login')) || targetElement.closest('.form-login__logo')) {
			loginBox.classList.remove('_show');
		}
		// Show/Hide Cart
		const headerCartOrder = document.querySelector('.order-cart');
		if(targetElement.closest('.cart-header__icon')) {
			e.preventDefault();
			headerCartOrder.classList.add('_active');
			document.querySelector('body').classList.add('_lock');
		} else if(targetElement.closest('.order-cart__close')) {
			headerCartOrder.classList.remove('_active');
			document.querySelector('body').classList.remove('_lock');
		} else if(!targetElement.closest('.order-cart__inner')) {
			headerCartOrder.classList.remove('_active');
			document.querySelector('body').classList.remove('_lock');
		}
		// Button Add to Cart
		if(targetElement.classList.contains('links-product__link-addtocart')) {
			e.preventDefault();
			const productId = targetElement.closest('.card-product').dataset.pid;
			addToCart(targetElement, productId);
		}
		// Button Delete From Cart
		if(targetElement.classList.contains('table-order-cart__link-delete')) {
			e.preventDefault();
			const productId = targetElement.closest('.table-order-cart__body-tr').dataset.cartPid;
	...
	function loadProducts(data) {
		const productsItems = document.querySelector('.products-home__cards');
		data.products.forEach(item => {
			...
			let productTemplateStart = `<article data-pid="${productId}" class="products-home__card card-product">`;
			let productTemplateAfterStart = `<div class="card-product__inner">`;
			let productTemplateBeforeEnd = `</div>`;
			let productTemplateEnd = `</article>`;

			let productTemplateImage = `
				<a class="card-product__img-box" href="${productUrl}">
					<img class="card-product__img" src="images/products/${productImg}" alt="${productTitle}">
				</a>
			`;

			let productTemplateBodyLinks = `
				<div class="card-product__links links-product">
					<ul class="links-product__list">
						<!-- Cart -->
						<li class="links-product__list-item">
							<a class="links-product__list-link links-product__link-addtocart _icon-cart" href="#"></a>
						</li>
						<!-- Heart -->
						<li class="links-product__list-item">
							<a class="links-product__list-link _icon-heart" href="#"></a>
						</li>
						<!-- Search -->
						<li class="links-product__list-item">
							<a class="links-product__list-link _icon-search" href="#"></a>
						</li>
					</ul>
				</div>
			`;

			let productTemplateProductName = `
				<a class="card-product__title" href="${productUrl}">${productTitle}</a>
			`;

			let productTemplatePrice = `
				<span class="card-product__price _title-price">${Number(productPrice).toFixed(2)}</span>
			`;

			let productTemplate = '';
			productTemplate += productTemplateStart;
			productTemplate += productTemplateAfterStart;
			productTemplate += productTemplateImage;
			productTemplate += productTemplateBodyLinks;
			productTemplate += productTemplateBeforeEnd;
			productTemplate += productTemplateProductName;
	...
	function addToCart(productButton, productId) {
		if(!productButton.classList.contains('_hold')) {
			...
			const productImg = product.querySelector('.card-product__img-box');
			...
			productImgFly.setAttribute('class', '_fly-image');
	...
	function updateCart(productButton, productId, productAdd = true) {
		...
		const cartQty = cartIcon.querySelector('span');
		...
		if(productAdd) {
			...
			if(!cartProduct) {
				const product = document.querySelector(`[data-pid="${productId}"]`);
				const cartProductImg = product.querySelector('.card-product__img-box').innerHTML;
				const cartProductTitle = product.querySelector('.card-product__title').innerHTML;
				const cartProductPrice = Number(product.querySelector('.card-product__price').innerHTML);
				const cartProductTableBody = document.querySelector('.table-order-cart__body');
				const cartProductContent = `
					<tr class="table-order-cart__body-tr" data-cart-pid="${productId}">
						<td class="table-order-cart__td table-order-cart__td--info td-info">
							<a class="td-info__img-box" href="#">
								${cartProductImg}
							</a>
							<div class="td-info__titles">
								<a class="td-info__title" href="#">${cartProductTitle}</a>
								<div class="td-info__subtitle">
									<span>SKU:</span>1995751877966
								</div>
							</div>
						</td>
						<td class="table-order-cart__td table-order-cart__td--qty"><span>1</span></td>
						<td class="table-order-cart__td table-order-cart__td--subtotal">
							<span class="table-order-cart__td-subtotal-price _title-price">${Number(cartProductPrice).toFixed(2)}</span>
							<a class="table-order-cart__link-delete _icon-delete" href="#"></a>
						</td>
						<td class="table-order-cart__td table-order-cart__td--delete">
					</tr>
				`;
				cartProductTableBody.insertAdjacentHTML('beforeend', `${cartProductContent}`);
			} else {
				const cartProductQuantity = cartProduct.querySelector('.table-order-cart__td--qty span');
				cartProductQuantity.innerHTML = ++cartProductQuantity.innerHTML;
				// Sum Price ==========================================
				const product = document.querySelector(`[data-pid="${productId}"]`);
				let qty = Number(cartProductQuantity.innerHTML);
				const cartProductPrice = Number(product.querySelector('.card-product__price').innerHTML);
				console.log(typeof cartProductPrice,cartProductPrice);

				const cartProductSum = cartProduct.querySelector('.table-order-cart__td-subtotal-price');
				cartProductSum.innerHTML = Number(cartProductPrice * qty).toFixed(2);
				console.log(typeof cartProductSum,cartProductSum);
				// ====================================================
			}
			...
			const cartProductPrice = Number(product.querySelector('.card-product__price').innerHTML);
			const cartProductSum = cartProduct.querySelector('.table-order-cart__td-subtotal-price');
			const cartProductDiff = Number(cartProductSum.innerHTML);
			cartProductSum.innerHTML = Number(cartProductDiff - cartProductPrice).toFixed(2);
			// ================================================

			if(!parseInt(cartProductQuantity.innerHTML )) {
				cartProduct.remove();
			}
			const cartQuantityValue = --cartQty.innerHTML;
			if(cartQuantityValue) {
				cartQty.innerHTML = cartQuantityValue;
			} else {
				cartQty.remove();
			}
		}
	...
	if(document.querySelector('.info-order-cart__list-date')) {
		let options = { year: 'numeric', month: 'short', day: 'numeric' };
		let today  = new Date();
		const boxDate = document.querySelector('.info-order-cart__list-date');
		const curDate = today.toLocaleDateString("en-US", options);
		boxDate.innerHTML = curDate;
	}

}

// Tabs Login/Register
if (document.querySelector('.tabs-login')) {
	const loginNavItems = document.querySelectorAll('.tabs-login__list-link');
	const loginContent = document.querySelectorAll('.form-login');

	loginNavItems.forEach(
	...
	document.querySelector('.tabs-login__list-link').click();
}

// Tabs Product Description/Reviews
if (document.querySelector('.footer-shop-product__contents')) {
	const tabsNavProduct = document.querySelectorAll('.tabs-product-footer__list-link');
	const contentDescrProduct = document.querySelectorAll('.footer-shop-product__content');

	tabsNavProduct.forEach(
		(item) => item.addEventListener('click', function(e) {
			e.preventDefault();
			const id = e.target.getAttribute('href').replace('#', '');
	...
	document.querySelector('.tabs-product-footer__list-link').click();
}

// Home Filter Range
if(document.querySelector('.filter-price__range')) {

	const priceFilter = document.querySelector('.filter-price__range');
	...
	priceFilter.noUiSlider.on('update', function (values, handle, unencoded, isTap, positions) {
    priceNodes[handle].innerHTML = '$' + Number(values[handle]).toFixed(0);
	});
}
...
if(document.querySelector('.slider-intro')) {
	const homeTopSlider = new Swiper('.slider-intro__swiper', {
		slidesPerView: 1,
		loop: true,
		spaceBetween: 0,
		parallax: true,
...
// Sliders Shop Product ====================================================
if(document.querySelector('.product-shop__sliders')) {
	// Product Subslider
	const subSliderProduct = new Swiper('.subslider-product__body', {
		slidesPerView: 4,
		spaceBetween: 10,
		breakpoints: {
			580: {
	...
	// Product Slider
	const sliderProduct = new Swiper('.slider-product__body', {
		slidesPerView: 1,
		thumbs: {
...
// Slider Released Products ====================================================
if(document.querySelector('.slider-released__body')) {
	const releasedSlider = new Swiper('.slider-released__body', {
		slidesPerView: 1.3,
		loop: true,
		spaceBetween: 15,
		speed: 1000,
		pagination: {
			el: ".slider-released__pagination",
			type: 'bullets',
		},
		breakpoints: {
			0: {
...
// if (document.querySelector('.info-news__btn')) {
// 	const btnRecentNews = document.querySelectorAll('.info-news__btn');

// 	btnRecentNews.forEach(
// 		(itemBtn) => itemBtn.addEventListener('click', function() {
// 			itemBtn.previousElementSibling.classList.toggle('info-news__text--hidden');
// 		})
// 	);
// }
// ===========================================================================
...
// 	const mixer = mixitup('.arrivals-wrapper');
...
if(spoilersArray.length > 0) {
	...
	function setSpoilerAction(e) {
		...
		if (el.hasAttribute('data-spoiler') || el.closest('[data-spoiler]')) {
			...
			if (!spoilersBlock.querySelectorAll('._slide').length) {
				...
				_slideToggle(spoilerTitle.nextElementSibling, 400);
			}
			e.preventDefault();
		}
	}
	...
	function hideSpoilersBody(spoilersBlock) {
		...
		if (spoilerActiveTitle) {
			...
			_slideUp(spoilerActiveTitle.nextElementSibling, 400);
		}
	}
}
